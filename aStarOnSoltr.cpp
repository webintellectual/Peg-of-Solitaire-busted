...
int calHeuristic(vector<vector<int> > state){
    int ans=0;
    for(int i=0; i<7; i++){
        for(int j=0; j<7; j++){
            if(state[i][j]==1){
...
struct Node{
    ...
    Node(){
        ...
        parent = NULL;
        pathCost=0;
        h = calHeuristic(state);
        f = pathCost+h;
    }
    Node(vector<vector<int> > st, Node* prt, int pCost, int hr){
        state = st;
        parent = prt;
        pathCost = pCost;
        h = hr;
            ...
            if(nd->state[i][j]==1){
                for(int k=0; k<4; k++){
                    ...
                    if(nd->state[c2i][c2j]==0){
                        ...
                        int hr = nd->h;
                        hr -= heuristic(i,j);
                        hr -= heuristic(c1i,c1j);
                        hr += heuristic(c2i,c2j);
                        Node* chld = new Node(cpy,nd,(nd->pathCost)+1,hr);
